...
function pentagon(x, y, radius, rotation, ctx) {
  for (var i = 0; i < 5; i++) {
    ...
    ctx.lineTo(Math.cos(ang) * radius + x, Math.sin(ang) * radius + y);
  }
  ctx.closePath();
}
...
function main() {
      ...
      case "GAME START":
        ...
        var foundAlert = setInterval(function () {
          ...
          var x = setInterval(function () {
            ...
            ctx.strokeText(
              num.toString(),
              canvas.width / 2 - ctx.measureText(num.toString()).width / 2,
              200
            );
            num--;
          }, 1000);
  ...
  function setupHome() {
    ...
    var genMainScreen = setInterval(drawShape, 3000);
    ...
    function playDown() {
      ws.send(
        JSON.stringify(["play", document.getElementById("nameInput").value])
      );
    ...
    canvas.addEventListener(
      ...
      function (e) {
        ...
        if (
          currX >= canvas.width / 2 - playTextWidth / 2 &&
          currX <= canvas.width / 2 + playTextWidth / 2 &&
          currY <= 250 &&
          currY >= 250 - playTextHeight
        ) {
          ws.send(
            JSON.stringify(["play", document.getElementById("nameInput").value])
          );
        ...
        if (
          currX >= canvas.width / 2 - rulesTextWidth / 2 &&
          currX <= canvas.width / 2 + rulesTextWidth / 2 &&
          currY <= 350 &&
          currY >= 350 - rulesTextHeight
        ) {
          if (inGame == false && rulesShown == false) {
            displayRules();
          } else if (inGame == false && rulesShown == true) {
